    ...
    async def execute(self, payload: dict) -> dict:
        ...
        try:
            ...
            if not all([action, api_key, service_id]):
                return {
                    "status": "error",
                    "message": "Missing required payload: action, api_key, service_id"
                }
            ...
            if action == "ADD_ENV_VAR":
                ...
                if not key or value is None:
                    return {
                        "status": "error",
                        "message": "Missing env_key or env_value for ADD_ENV_VAR"
                    }

                url = f"https://api.render.com/v1/services/{service_id}/env-vars"
                # This call overwrites, so we must first get, then append, then set.
                # For simplicity, we'll just add one. A real implementation is more complex.
                # NOTE: Render's API updates variables one by one or as a bulk set.
                # Let's use the bulk set method for safety.
                ...
                url = f"https://api.render.com/v1/services/{service_id}/logs?limit=100"
